...
%
%  isTPX parameter define which HW tracker will be used to do the calibration. 0 = TPx-Mini, 1 = TPx
%
%  isBuiltIn parameter defines two calibration types for the TRACPixx/mini.
%     0. Chin-rest type that uses the VPixx method (13 points).
%     1. Remote type that uses the device's built-in calibration (16 points).
%
%  screenNumber parameter sets to which screen the calibration will be
%  done, if the platform supports multi screen.
%
%
% History:
%
% Nov 1, 2017  dml     Written
% Oct 1, 2018  richard schweitzer, cleaned up and optimized for lab usage
% 
%WaitSecs(10);
...
%% TO DO:
% - throw out fakeHW, isTPX and isBuiltIn related code
% - make input variables: initialize TPx, screen-pointer, screen-bgcolor, 
% ledintensity, irissize, console mode, number of calibration points
% - 



%% Step 1, Initialize TRACKPixx 
    ...
    if ((t2 - t) > 1/60) % Just refresh at 60Hz.
        ...
        if left_rec(1) ~= 0
            if right_rec(1) ~= 0
                ...
                text_to_draw = ' Right click (red) on the right eye on the screen (physical left eye). C or middle mouse to clear. Escape to exit.';
            end
        else
            text_to_draw = ' Left click (blue) on the left eye on the screen (physical right eye). C or middle mouse to clear. Escape to exit.';
        end
...
while ~finish_calibration
    ...
    end % of evaluation of calibration
